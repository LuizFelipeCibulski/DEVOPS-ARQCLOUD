...
func main() {
	...
	var sqsSvc *sqs.SQS
	if sqsQueueURL != "" {
		awsEndpoint := os.Getenv("AWS_ENDPOINT_URL") // ex: http://ministack:4566
		...
		if awsEndpoint != "" {
			// Aponta o SDK para o emulador local (ministack) em vez da AWS real.
			// O SDK Go NÃO usa a QueueUrl para decidir o host de conexão — ele
			// calcula o endpoint pela Region, por isso é preciso setar Endpoint
			// explicitamente aqui quando existe AWS_ENDPOINT_URL.
			...
			cfg.DisableSSL = aws.Bool(true) // ministack roda em http, não https
		}
